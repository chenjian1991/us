 ...
 * @Last Modified time: 2018-11-30 15:41:46
 */

import { storage,getDecimalsNum,getUTCDay,getUTCHour,getUTCMinutes,isSameMinutes,getLastUTCMinutes,getHOLCObj,getCurrentMinutes,isSameUTCDay} from "@/lib/utils"
...
Datafeeds.UDFCompatibleDatafeed.prototype.subscribeBars = function(symbolInfo, resolution, onRealtimeCallback, subscriberUID, onResetCacheNeededCallback) {
    ...
    KlineSSE.onerror = function (event) {
        console.log('K线断开3s后重连^_^')
        setTimeout(function(){
            queryTime = getLastUTCMinutes()
            KlineSSE = new EventSource(`${baseURL}/quote/realTime.stream?symbol=${currentSymbol}&${currentSymbol}_least=1&${currentSymbol}_startTime=${queryTime}`)
        }.bind(this),3000)
        // handle error event
    };
        ...
        switch (resolution) {
            ...
            default: 
                ...
                if(data.dateTime < currentMinutes){
                    ...
                    if(isSameUTCDay(data.dateTime,lastMinutesQuote.dateTime)){//是否跨天
                        //第一个快照新的成交量
                        currentMinutesVolme = data.volume - lastMinutesQuote.volume
                    }else{
                        currentMinutesVolme = data.volume
                    }
                    //数组清空  放入新的快照
                    currentMinutesArr = []
                    currentMinutesArr.push(data)
