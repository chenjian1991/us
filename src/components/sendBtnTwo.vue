<template>
    <div class="btn_contain">
        <Button ref="btn"  :disabled='empty' :class="fatherClass" v-show="show" @click="sendCode" class="sendMs" type="primary">{{$t('regSend')}}</Button>
        ...
        methods: {
             ...
             let _that = this
                getApiLoin(ipQuery,'').then((res)=>{
                    if(res.resultcode==200){
                        _that.ipCountry = res.result.Country;
                        if(_that.ipCountry=='中国'){
                         ...
                         _that.ipCountry='中国'//ip查询失败的时候默认中国
                    }
                }).catch((error)=>{//当ip获取失败都时候默认是谷歌验证
                    console.log('e')
                    _that.ipCountry='中国'// 请求超时的还是把ip写死中国
                     console.log(_that.ipCountry)
                })
            },
            onloadCallback(){//谷歌人机验证方法
                let _that = this;
                let widgetId=grecaptcha.render('robot_div', {
            ...
            sendPostRequest(params,_that){//登录注册发送验证码 send
              ...
              SSOpostRequest(params,_that){//sso.send 发送短信验证码
                    postHeaderTokenBodyApi(ssoSend,_that.loginToken,params).then((res) =>{
                        if(res.code){
                            let code = res.code;
                            _that.$emit('sendCick',_that.$t(code))//触发父组件的方法，并传递参数给父组件；
                         if(res.code == '10013'){
